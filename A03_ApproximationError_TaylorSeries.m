...
% by analytical solution and thetesseroid field using the Taylor series expansion approach.
...
Taylor_grav=zeros(gridnumber,1);
tic
%Computing the gravitational contribution at computation grid nodes
    parfor grind=1:1:gridnumber
        Taylor_grav(grind)=Pf_Taylor_grav(griddata,grind,tessmodel,MEAN_EARTH_RADIUS_local,G_local,SI2MGAL_local);
    end 
AbsoError=(Taylor_grav - Analy_grav);
Differ_grav=[griddata(:,1:2) AbsoError];
toc
%Result variable name generation      
% eval([[tessmdNa,'_ObservH',num2str(Hp/1000),'km_Taylor'] '=Differ_grav;']);
% save('A03_Difference_grav_Taylorseries',[tessmdNa,'_ObservH',num2str(Hp/1000),'km_Taylor'],'-append');  %,'-append'